...
class TokenManager:
    def __init__(self):
        ...
        # NEW: where to store latest access token
        self.token_file_path = os.environ.get("ACCESS_TOKEN_FILE", "access_token.txt")
    ...
    def refresh_token(self):
        ...
        try:
            ...
            if access_token:
                try:
                    # NEW: write token to a file instead of environment variable
                    with open(self.token_file_path, "w", encoding="utf-8") as f:
                        f.write(access_token)

                    self.last_token_refresh = datetime.now()
                    logger.info(
                        f"✅ Access token refreshed and written to {self.token_file_path} at "
                        f"{self.last_token_refresh.strftime('%Y-%m-%d %H:%M:%S')}"
                    )
                    logger.info(f"   Token length: {len(access_token)} characters")
                    return True
                except Exception as file_err:
                    logger.error(f"❌ Failed to write access token to file {self.token_file_path}: {file_err}")
                    return False
