...
# NEW: where to read the token that scheduler writes
TOKEN_FILE_PATH = os.environ.get("ACCESS_TOKEN_FILE", "access_token.txt")

def load_access_token_from_file() -> str:
    """Read latest access token from file written by scheduler."""
    try:
        with open(TOKEN_FILE_PATH, "r", encoding="utf-8") as f:
            token = f.read().strip()
        if not token:
            logger.critical("Access token file %s is empty.", TOKEN_FILE_PATH)
            raise RuntimeError(f"Access token file {TOKEN_FILE_PATH} is empty.")
        return token
    except FileNotFoundError:
        logger.critical("Access token file %s not found.", TOKEN_FILE_PATH)
        raise RuntimeError(f"Access token file {TOKEN_FILE_PATH} not found.")
    except Exception as e:
        logger.critical("Failed to read access token from %s: %s", TOKEN_FILE_PATH, e)
        raise
